...
/** Bobr intro length (`BOBR_INTRO_TOTAL_MS`) plus buffer for compositor / CI jitter. */
export const bobrIntroSettleMs = 12_000
...
/** New run from title through full Bobr intro to starting village hub (`/content/village.png`). */
export async function goToVillageHub(page: Page) {
  ...
  await expect(hud.locator('img[src="/content/village.png"]')).toBeVisible({ timeout: bobrIntroSettleMs })
}
