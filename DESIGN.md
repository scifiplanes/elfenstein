...
Last updated: 2026-04-09 (hub innkeeper sprite scale debug)
...
- **Combat HUD**: **CombatIndicator** on the game viewport shows encounter enemies, current turn, and **Flee** / **Defend** (mirrors **R** / **F**). No separate encounter modal.
- **NPC actions**: on NPC turns, the NPC automatically attacks a party member (MVP targeting: lowest current HP, stable tie-break).
- **NPC hit vs party**: **d20 + NPC Speed** vs **10 + target PC Speed** decides hit/miss; **crit** on natural 20. Damage uses that NPC kind’s **base damage** and **damage type**, then **PC armor** (Blunt/Pierce/Cut only), **resistances**, and **Defend** bonuses (`npcTakeTurn`).
