...
- Rendering:
  ...
  - The NPC dialog panel is **anchored to the top of the 3D game viewport** (`GameViewport`): **horizontally centered** within that region, width capped (with horizontal padding), with a small top inset minus an **upward offset** proportional to viewport height (**100 CSS px** at **1080**-tall measured viewport, **`100 / STAGE_CSS_HEIGHT`**) plus a **downward** offset (**30 CSS px** at that height, **`30 / STAGE_CSS_HEIGHT`**); layout **re-measures** on resize and scroll so it stays aligned with the HUD **game** cell. The dialog **panel** uses **square corners** (**no** outer **`border-radius`** on the main box; control buttons may keep their own radii).
...
- **Frame presentation pipeline**:
  ...
  - **Paperdoll** mounts **only** in **`DitheredFrameRoot`’s `stageModalLayer`** (sibling of **`.interactiveHud`**, not under **`opacity: 0`**). The **NPC dialog** is **opened** from the same layer ( **`npcDialogFor`** ) but its DOM is **`createPortal`**’d to **`document.body`** so **`position: fixed`** and viewport **`getBoundingClientRect()`** math stay correct: **`FixedStageViewport`’s** **`transform: scale`** on **`.stage`** would otherwise make fixed descendants use that ancestor as their containing block and **mis-center** the panel. The modal layer still mounts **only while** **`paperdollFor`** or **`npcDialogFor`** (or title/death) applies, uses **`pointer-events: auto`**, and sits **above** the invisible hit HUD (`z-index`) where applicable. Modals are **not** part of the **`html2canvas`** capture tree.
