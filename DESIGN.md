...
- **Portrait stats presentation**: character vitals + status use a **compact bottom overlay inside the portrait frame**. **HP**, **STA**, **HUN**, **THR** each occupy one **row** in order **icon · abbreviated name · numeric value** (emoji icon + mono label + value). A **status** line below (**`Status: …`**) stays **single-line truncated** when long.
