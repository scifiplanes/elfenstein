...
- Rendering:
  ...
  - While the dialog is open, **`player/turn`**, **`player/step`**, and **`player/strafe`** dismiss it first (clear **`ui.npcDialogFor`** and F2 **`debugShowNpcDialogPopup`**, same as **`ui/closeNpcDialog`**), then run the movement attempt; stepping onto a **non-hostile** NPC can open the dialog again on that same action.
