...
Last updated: 2026-04-09 (NPC dialog gibberish: Deep Gnome / Mojibake / Zalgo procedural speech; quest NPC combat log lines; in-combat feed triage; Fire NPC tuning; combat tests; encounter turn preview + SFX)
...
- Rendering:
  ...
  - The NPC dialog header is **text-only** (**`{name} · {status}`**); there is **no** portrait/icon next to the title (in-world billboards remain unchanged). (the **language id** is **not** shown in the chrome—players infer it from **gibberish** copy). Dialog copy (**title**, **gibberish body**, **hint**, **Close**) uses the **`--buttonTitle*`** face (**Jim Nightshade** stack) while keeping the prior **pixel sizes** (**37 / 23 / 21 / 23** CSS px respectively for title / body / hint / Close). **Close** uses a **`2px`** border in **`--buttonTitleColor`** (same as its label) and **square** corners.
...
- **Frame presentation pipeline**:
  ...
  - **Modal interaction** matches the **NPC dialog** split for every gameplay popup: an **offscreen capture** variant feeds the **dithered** HUD texture; an **interactive** variant is **`createPortal`**’d to **`document.body`** inside **`GamePopup`’s** invisible **`modalPortalHitRoot`** (`opacity: 0`, high **`z-index`**) so hits win over **`FixedStageViewport`** **`transform: scale`** and **`CursorLayer`**, while **`position: fixed`** (where used) tracks the real viewport. **`DitheredFrameRoot`’s `stageModalLayer`** still mounts **`TitleScreen`**, **`TavernTradeModal`** (stub innkeeper trade), **`DeathModal`**, **`PaperdollModal`**, and **`NpcDialogModal`** only while the corresponding **`ui.*`** flags apply (hooks + portals); the layer keeps **`pointer-events: auto`** when present.
